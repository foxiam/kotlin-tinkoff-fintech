...
class SequenceServiceTest {
    ...
    private val service = SequenceService(listCar)
    ...
    fun `price sort test`() {
        ...
        assertEquals(sortedCar, service.getDescriptionAndSort().toList())
    }
    ...
    fun `body type filter test with return first 3 elements`() {
        ...
        val serviceForTestFilter = SequenceService(fourSameBodyType)

        var filteredCar = fourSameBodyType.subList(0, 3)

        assertEquals(filteredCar, serviceForTestFilter.getThreeFiltered { it.bodyType == "Sedan" }.toList())
    }
    ...
    fun `fuel consumption filter test with return first 3 elements`() {
        ...
        assertEquals(filteredCar, service.getThreeFiltered { it.fuelConsumption < 11}.toList())
    }
}